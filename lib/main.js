...
function dispatchBy( fcns, types, data, nargs, nin, nout ) {
	...
	if ( isFunction( fcns ) ) {
		...
		throw new TypeError( format( 'null5s', fcns ) );
	}
	if ( !isCollection( types ) ) {
		throw new TypeError( format( 'null3A', types ) );
	}
	...
	if ( !isPositiveInteger( nargs ) ) {
		throw new TypeError( format( 'null5u', nargs ) );
	}
	if ( !isNonNegativeInteger( nin ) ) {
		throw new TypeError( format( 'null5v', nin ) );
	}
	if ( !isNonNegativeInteger( nout ) ) {
		throw new TypeError( format( 'null5w', nout ) );
	}
	narrays = nin + nout;
	if ( narrays === 0 ) {
		throw new Error( format( 'null0h' ) );
	}
	if ( nargs !== narrays+1 ) {
	...
	*/
	function dispatcher() {
		...
		if ( !isFunction( clbk ) ) {
			throw new TypeError( format( 'null2n', clbk ) );
		}
		arrays = [];
		dtypes = [];
		for ( i = 0; i < narrays; i++ ) {
		...
		if ( idx < 0 ) {
			throw new TypeError( format( 'null0p' ) );
		}
